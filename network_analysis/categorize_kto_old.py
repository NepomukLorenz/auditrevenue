...
def _extract_top_gegenkonten(bewegungen_df: pd.DataFrame, konto: str, kto, gkto_name, soll, n: int = 3) -> str:
    gefiltert = bewegungen_df[bewegungen_df[kto] == konto]
    gruppiert = (
        gefiltert.groupby(gkto_name)[soll]
        .sum()
...
def _call_ai_kategorie_bestimmen(kontonummer: str, kontobezeichnung: str, gegenkontoinfo: str = "", nachbarkonten: str = "") -> str:
    system_msg = {
        "role": "system",
        "content": (
...
[Aufwand, Zahlungsmittel, Umsatzerlöse, Sonstige Erlöse, Debitoren, Sonstige Forderungen, Kreditoren, Verrechnungskonten, Umsatzstueuer, Sonstige Aktiva, Sonstige Passiva, Eröffnungskonten]
...
def categorize_kto(
    ...
    for _, row in df_konten.iterrows():
        ...
        if df_bewegungen is not None:
            gegen_info = _extract_top_gegenkonten(df_bewegungen, konto, df_bewegungen_kto, df_bewegungen_gkto_name, df_bewegungen_soll)

        nachbarkonten = _get_nachbarkonten(df_konten, konto, df_konten_kto, df_konten_kto_name, n=3)
    ...
    agg_categorized = df_bewegungen.merge(
        df_result[[df_konten_kto, "kto_kategorie"]],
        on=df_konten_kto,
        how="left"
    )
